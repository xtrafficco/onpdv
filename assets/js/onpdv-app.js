...
function openRaioX(){
  ...
  if(!RAIOX_LOADING){
    // O CSS vem junto com o modulo, nao no <head> do index: quem nunca abre o
    // Raio-X nao baixa mais o estilo dele no login. Esperamos os dois antes de
    // renderizar para a aba nao piscar sem estilo. (O sw.js ja cacheia ambos,
    // entao offline continua abrindo.)
    RAIOX_LOADING=Promise.all([
      new Promise((resolve)=>{
        if(document.querySelector('link[data-onpdv-css="raiox"]')) return resolve();
        const l=document.createElement('link');
        l.rel='stylesheet'; l.href='assets/css/onpdv-raiox.css';
        l.dataset.onpdvCss='raiox';
        // Falha de CSS nao impede abrir: a tela sai sem enfeite, mas abre.
...
// ===================== FRENTE DE CAIXA (PDV) =====================
// Daqui ate a secao CLIENTES e o caixa: teclado, carrinho, pagamento, entregas,
// retirada, crediario e impressao. Sao ~2.200 linhas e 196 funcoes pdv*.
//
// Este banner existia? Nao. Ate 18/09/2026 tudo isto vivia debaixo do banner da
// "VITRINE - 2a TELA", que na verdade termina em cfdPay, logo acima: 13 funcoes e
// ~70 linhas. Quem lesse os banners para achar o que extrair do bundle concluiria
// que a 2a tela pesa 2.318 linhas, quando pesa 70. Um banner errado custa mais caro
// que nenhum, porque a pessoa confia nele.
...
async function loadCompany(){
  ...
  fillConfigForms();
  try{ renderCollectionsPixCard(); }catch(_){}
}
...
// Salva um pedaço do pdv_config PRESERVANDO o resto. Antes montava o objeto do zero
// com as chaves que a tela conhece e mandava um update inteiro — o que apagava
// silenciosamente `pedido_site` (gravado por erp_site_config_salvar) e `min_auto`
// (lido por recompute_min_stock). Passava despercebido só porque o pdv_config ainda
// não tinha essas chaves: bastaria configurar o pedido pelo site e depois salvar a
// balança para perder a configuração do site.
async function savePdvConfig(patch, okMsg){
  let atual={};
  try{
    const { data, error } = await sb.from('company_settings').select('pdv_config').eq('id',1).maybeSingle();
    if(error) throw error;
    if(data&&data.pdv_config&&typeof data.pdv_config==='object') atual=data.pdv_config;
  }catch(e){ toast('Erro: '+(e&&e.message||e),true); return; }
  const next=Object.assign({}, atual);
  for(const chave of Object.keys(patch||{})) next[chave]=Object.assign({}, atual[chave], patch[chave]||{});
...
// ---- Liga/desliga da cobrança automática por PIX no WhatsApp (#3) ----
function collectionsPixOn(){ return !!(COMPANY&&COMPANY.pdv_config&&COMPANY.pdv_config.collections_pix&&COMPANY.pdv_config.collections_pix.enabled); }
function renderCollectionsPixCard(){
  const st=$('#cpxStatus'), body=$('#cpxBody'); if(!body) return;
  const on=collectionsPixOn();
  if(st){ st.textContent=on?'ligada':'desligada'; st.className='chip '+(on?'ok':''); }
  body.innerHTML = on
    ? '<button class="btn ghost red" data-onclick="toggleCollectionsPix(false)">Desligar cobrança automática</button>'
    : '<button class="btn green" data-onclick="toggleCollectionsPix(true)">Ligar cobrança automática</button>';
}
window.toggleCollectionsPix = async (on)=>{
  on = (on===true||on==='true');
  if(on && !await uiConfirm('Ligar a cobrança automática por PIX?\n\nOs lembretes de crediário vencido no WhatsApp passarão a incluir um PIX da parcela, e o pagamento baixa a parcela sozinho. Confira antes que o provedor de WhatsApp está configurado e que o texto da mensagem está do seu jeito.',{okText:'Ligar'})) return;
  const cur=(COMPANY&&COMPANY.pdv_config)||{};
  const next=Object.assign({}, cur, { collections_pix:Object.assign({}, cur.collections_pix, { enabled:on }) });
  const { error } = await sb.from('company_settings').update({ pdv_config:next }).eq('id',1);
  if(error){ toast('Erro: '+error.message,true); return; }
  COMPANY.pdv_config=next;
  try{ refCachePut('company', COMPANY, true); }catch(_){}
  toast(on?'Cobrança automática por PIX LIGADA':'Cobrança automática por PIX desligada');
  renderCollectionsPixCard();
};
// ---- frete por bairro ----
...
async function loadDecisionCenter(){
  ...
  // Dinheiro preso: PIX pago (crediário/entrega/venda) que ainda não casou no sistema.
  try{
    const { data:ph } = await sb.rpc('erp_pix_health',{ p_stuck_min:20 });
    const presos=(ph&&ph.presos)||[];
    if(presos.length){
      const tot=presos.reduce((a,p)=>a+(+p.valor||0),0);
      alerts.unshift(dcTask('PIX pago aguardando confirmação', `${presos.length} pagamento(s) · ${BRL(tot)} já pagos no Mercado Pago ainda não entraram no caixa. Reconfira no PDV.`, 'caixa', 'warn'));
    }
  }catch(_){}
...
async function renderTerminals(){
  ...
  $('#termBody').innerHTML = TERMINALS.map(t=>{
    ...
    <tr>
      ...
      <td>${t.provider_terminal_id?`<code style="font-size:12px">${esc(t.provider_terminal_id)}</code>`:'<span class="chip warn" style="font-size:10px" title="Sem Device ID o Mercado Pago recusa a cobrança nesta maquininha">sem Device ID · não cobra</span>'}</td>
      <td>${storeCell}</td>
  ...
  modal(`
    ...
    <div class="m-body">
      ...
      <div class="field"><label class="lbl" for="tfDevice">Device ID do Point</label>
        ...
        <p class="muted" style="font-size:12px;margin-top:4px">No aparelho, ative o modo <b>PDV/Integrado</b> e copie o Device ID no app do vendedor do Mercado Pago. <b>É obrigatório</b>: sem ele o Mercado Pago recusa a cobrança nesta maquininha.</p></div>
